...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 236, mthresh=7, close = 0, use_otsu=False, 
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                if custom_downsample > 1:
                    ...
                    assert 2==3
                    # patch_PIL.save('/data/beidiz/CLAM1/temp/'+str(y)+'_'+str(x)+'.jpg')
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if vis_level < 0:
            vis_level = self.wsi.get_best_level_for_downsample(8)
        ...
        for idx in range(len(coords)):
            ...
            if score >= threshold:
                ...
                img_block =  np.zeros((patch_size[1], patch_size[0], 3), dtype=np.uint8)
        ...
        if alpha < 1.0:
            ...
            blurred = cv2.GaussianBlur(equalized, (19,19), 0)
            edges = cv2.Canny( blurred, 150, 200)

#            Find contours
            contours, hierarchy = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

            # Optionally, smooth the contours
            smoothed_contours = [cv2.approxPolyDP(cnt, 0.02*cv2.arcLength(cnt, True), True) for cnt in contours]

            # Draw the contours on the original image
            img = cv2.drawContours(img_ori, smoothed_contours, -1, (0, 255, 0), 2)
            # img = cv2.drawContours(img, smoothed_contours, -1, (0, 255, 0), 2)
