...
class CLAM_MB(CLAM_SB):
    def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        self.size_dict = {"small": [args.input_dim, 256, 256], "big": [args.input_dim, 512, 384]}
        size = self.size_dict[size_arg]
        fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
        if dropout:
            fc.append(nn.Dropout(0.25))
    ...
    def forward(self, x_path, x_omic=None, label=None, instance_eval=False, return_features=False, attention_only=False):
        h = x_path
        device = h.device
        A, h = self.attention_net(h.squeeze())  # NxK        
        A = torch.transpose(A, 1, 0)  # KxN
