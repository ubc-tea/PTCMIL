...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['ptcmil', 'clam', 'transmil','abmil','dsmil','ilra'], 
...
if 'survival' in args.task:
    ...
    if combined_study in ['tcga_blca', 'tcga_brca', 'tcga_luad', 'tcga_crc']:
...
def main(args):
    ...
    for i in folds:
        ...
        project="clustering",
        name="fold_"+ str(i),
        group=args.model_type + '_' + args.split_dir.split('/')[-1].split('_')[-1] + '_reg' + str(args.reg) + '_lr' + str(args.lr),
        job_type="training",  # Optionally differentiate between job types
