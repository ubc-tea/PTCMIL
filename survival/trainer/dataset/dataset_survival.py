...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path = 'dataset_csv/ccrcc_clean.csv', mode = 'omic', apply_sig = False, 
        ...
        slide_data = slide_data.dropna(subset=['censorship'])
        slide_data = slide_data.dropna(subset=['oncotree_code'])
        ...
        is_nan_censorship = uncensored_df['censorship'].isna()
        if sum(is_nan_censorship) > 0:
            print('# of NaNs in Censorship col, dropping:', sum(is_nan_censorship))
            uncensored_df = uncensored_df[~is_nan_censorship]
