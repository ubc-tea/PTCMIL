...
class TCFormer(nn.Module):
    def __init__(
            self, img_size=224, in_chans=384, embed_dims=[64, 128, 256, 512],
        ...
        self.k = k
        self._fc1 = nn.Sequential(nn.Linear(1024, 384), nn.ReLU())
    ...
    def get_divisible_value(self, value, divisor):
        return math.ceil(value / divisor) * divisor
    
    def get_larger_value(self, value, min_value):
    ...
    def forward(self, x):
        ...
        _H = self.get_larger_value(_H, 30)
        _W = self.get_larger_value(_W, 30)
        add_length = _H * _W - H
        while add_length > x.size(1):
            x = torch.cat([x, x[:,:add_length,:]],dim = 1) 
            add_length = _H * _W - x.size(1)
        x = torch.cat([x, x[:,:add_length,:]],dim = 1) 

        x = self.pos_layer(x, _H, _W)

        outs = self.forward_features(x)
        x = outs[-1]['x'].mean(dim=1)
        x = self.head(x)
        Y_prob = F.softmax(x, dim=1)
        Y_hat = torch.argmax(x, dim=1)
        result_dict = {}
        return x, Y_prob, Y_hat, None, result_dict
...
class tcformer(TCFormer):
    def __init__(self, **kwargs):
        super().__init__(
            embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
            # embed_dims=[384, , 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
