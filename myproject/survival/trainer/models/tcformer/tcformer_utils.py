...
def get_top_patches(idx, weights):
    norm_weight = weights.squeeze().detach().cpu()
    print(norm_weight.size())
    
    idx = idx.squeeze().squeeze().detach().cpu()
    print(idx)

    


    # Initialize a dictionary to store nodes by clusters
    cluster_dict = defaultdict(list)

    # Populate the dictionary with node indices and their corresponding weights
    for id, (cluster, weight) in enumerate(zip(idx, norm_weight)):
        cluster_dict[cluster.item()].append((weight.item(), id))

    # Find top 10 nodes with highest weights for each cluster
    top_10_nodes_per_cluster = {}
    for cluster, weight_idx_list in cluster_dict.items():
        # Sort by weight in descending order and take top 10
        top_10 = sorted(weight_idx_list, key=lambda x: x[0], reverse=True)[:10]
        # Store only the indices of these top 10 nodes
        top_10_nodes_per_cluster[cluster] = [idx for _, idx in top_10]

    # Print or use the result
    destination_path_0 = os.path.join('/data/beidiz/MOTCat/visualization/temporary/TCGA-05-4415-01Z-00-DX1','top')
    if os.path.exists(destination_path_0) == False:
        os.mkdir(destination_path_0)
    for cluster, nodes in top_10_nodes_per_cluster.items():
        print(f"Top 10 nodes in cluster {cluster}: {nodes}")
        for i in nodes:
            source_path = os.path.join('/data/beidiz/MOTCat/visualization/temporary/TCGA-05-4415-01Z-00-DX1/cluster_' +str(cluster) , f"{i}.jpg")
            destination_path_0 = os.path.join('/data/beidiz/MOTCat/visualization/temporary/TCGA-05-4415-01Z-00-DX1','top','cluster_' +str(cluster))
            if os.path.exists(destination_path_0) == False:
                os.mkdir(destination_path_0)
         
            destination_path = os.path.join(destination_path_0, f"{i}.jpg")
            
            if os.path.exists(source_path):
                assert 3==4
                shutil.copy(source_path, destination_path)
            else:
                print(f"Image {i}.jpg does not exist in the source folder.")
    assert 2==3
    return True
...
def cluster_dpc_knn(x, cluster_num, k=5, token_mask=None):
    ...
    with torch.no_grad():
        ...
        if N > 50000:
            chunk_size = 256  # Set the chunk size to a smaller value

            dist_matrix_list = []
            
            for i in range(0, x.size(0), chunk_size):
                x_chunk = x[i:i+chunk_size]
                dist_chunk = torch.cdist(x_chunk, x) / (C ** 0.5)
                dist_matrix_list.append(dist_chunk)
                del x_chunk,dist_chunk

            dist_matrix = torch.cat(dist_matrix_list, dim=0)

            # Delete intermediate tensors from torch memory
            del dist_matrix_list
        ...
        # print(dist_matrix.size(), mask.size(), dist_max.size())
        dist_matrix = dist_matrix.squeeze(0)
        mask = mask.squeeze(0)
        dist_max = dist_max.squeeze(0)
        ...
        for i in range(0, dist_matrix.size(0), chunk_size):
            dist_chunk = dist_matrix[i:i+chunk_size]
            # mask_chunk = (density[:, None, i:i+chunk_size] > density[:, i:i+chunk_size, None]).type(x.dtype)
            mask_chunk = mask[i:i+chunk_size].type(x.dtype)

            dist_chunk = dist_chunk * mask_chunk + dist_max * (1 - mask_chunk)
            dist_chunk, index_parent_chunk = dist_chunk.min(dim=-1)

            dist_list.append(dist_chunk)
            index_parent_list.append(index_parent_chunk)

            # Delete intermediate tensors from torch memory
            del dist_chunk, mask_chunk

        dist = torch.cat(dist_list, dim=0)

        # Delete remaining tensors from torch memory
        del dist_list, index_parent_list, mask, dist_max
        # index_parent = torch.cat(index_parent_list, dim=0)
        ...
        # Delete dist tensor from torch memory
        del dist, dist_matrix
...
def merge_tokens(x, idx_cluster, cluster_num, token_weight=None):
    ...
    idx = idx_cluster + idx_batch * cluster_num # [B, N], [1,1,5,4,6,..,7]
    ...
    # get_top_patches(idx,norm_weight)
    ...
    # print(out_dict['agg_weight'].size())
    # return out_dict
    return x_merged

def merge_tokens_mean(x, idx_cluster, cluster_num):
    # Initialize an empty tensor to store the mean of features for each cluster
    cluster_means = torch.zeros(cluster_num, x.size(-1))
    # Calculate the mean of features for each cluster
    for i in range(cluster_num):
        # Select rows where the assignment tensor is equal to the current cluster index
        cluster_data = x[idx_cluster == i]
        # Calculate the mean along the rows
        cluster_means[i] = cluster_data.mean(dim=0)
    cluster_means = cluster_means.unsqueeze(0).cuda()
