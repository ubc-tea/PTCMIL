...
from .tcformer_utils import (
    merge_tokens, merge_tokens_mean, cluster_dpc_knn, token2map,
...
class CTM(nn.Module):
    ...
    def forward(self, x):
        ...
        return idx_cluster, cluster_num, x_merge

# CTM block
class CTM_w_clustering(nn.Module):
    def __init__(self, cluster_num, dim_out):
        super().__init__()
        self.dim_out = dim_out
        # self.conv = TokenConv(in_channels=embed_dim, out_channels=dim_out, kernel_size=3, stride=2, padding=1)
        self.norm = nn.LayerNorm(self.dim_out)
    ...
    def forward(self, x, idx_cluster):
        ##----> Weighted sum
        x = self.norm(x)
        token_score = self.score(x)
        token_weight = token_score.exp()
        x_merge = merge_tokens(x, idx_cluster, self.cluster_num, token_weight)
        ##----> Mean of each cluster
        # x_merge = merge_tokens_mean(x, idx_cluster, self.cluster_num)
